...
#define IPS_EXT		".ips"
// 把使用的 OPTION_IPSPATH 或者 machine.options().value(OPTION_IPSPATH) 全部替换为 IPS_ROOT_PATH，
#define IPS_ROOT_PATH "ips"
#define CRC_STAG	"CRC("
#define CRC_ETAG	")"
...
static bool parse_ips_patch(running_machine &machine, const char *patch_name, const tiny_rom_entry *romp)
{
    ...
    while (fpDat.gets(buffer, sizeof(buffer)))
    {
		...
		// 修改开始：查找是否已经存在该 ROM 的 entry
		ips_entry* current_entry = nullptr;
		for (auto &e : s_ips_list)
		{
			if (core_stricmp(e->rom_name.c_str(), rom_name) == 0)
			{
				current_entry = e.get();
				break;
			}
		...
		{
			// 已存在：执行追加
			osd_printf_info("IPS: Appending additional IPS '%s' to existing ROM entry '%s'\n", ips_name_str.c_str(), rom_name);
			

			current_entry->ips_name.append(",").append(ips_name_str);
			
			// 将补丁数据追加到同一个 chunks 向量中
			if (!load_ips_file(machine, current_entry->chunks, ips_dir_str.c_str(), ips_name_str.c_str()))
			{
				osd_printf_info("IPS: ERROR - load_ips_file (append) FAILED for '%s'\n", ips_name_str.c_str());
				s_error_string += string_format("ERROR: %s/%s: IPS data append fail\n", ips_dir_str, ips_name_str);
			}
		}
		else 
		{
			...
			osd_printf_info("IPS: Loading first IPS for ROM '%s': '%s'\n", rom_name, entry->ips_name.c_str());
